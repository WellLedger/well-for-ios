(function() {
    ...
    // 形象清单/名称/强调色统一取自建模注册表（js/pets/*.js），此处不再单独维护
    const PET_VARIANTS = (window.PET_MODEL_REGISTRY && window.PET_MODEL_REGISTRY.keys().length)
        ? window.PET_MODEL_REGISTRY.keys() : ['default'];
    const PET_VARIANT_NAMES = {};
    const PET_VARIANT_ACCENT = {};
    const PET_VARIANT_TAG = {};
    PET_VARIANTS.forEach(function (k) {
        var m = window.PET_MODEL_REGISTRY ? window.PET_MODEL_REGISTRY.get(k) : null;
        PET_VARIANT_NAMES[k] = (m && m.name) || k;
        PET_VARIANT_ACCENT[k] = (m && m.accent) || '#F3A16E';
        PET_VARIANT_TAG[k] = (m && m.tag) || '';
    });
    // 名字 + 可选括号标注（如「雪妮（默认）」），仅在展示位置使用，提示文案仍用纯名字
    function variantNameHtml(v) {
        var tag = PET_VARIANT_TAG[v];
        return PET_VARIANT_NAMES[v] + (tag ? '<i class="pet-skin-name-tag">（' + tag + '）</i>' : '');
    }
    // 旧版形象键 -> 新建模键 的迁移映射（历史数据/localStorage 兼容）
    const PET_VARIANT_LEGACY_MAP = {
        'orange': 'taozi', 'cowcat': 'heitang', 'tabby': 'huahua',
        'blue': 'huihui', 'ice': 'xiaobai', 'milk': 'xueqiu'
    };
    function normPetVariant(v) { return PET_VARIANT_LEGACY_MAP[v] || v; }
    ...
    /* 切换形象后把本地宠物状态缓存里的 variant 一起改掉。
       缓存不跟着改的话，之后任何一次「缓存优先」渲染（冷启动 hydrate、
       晚到的旧缓存）都会把界面画回旧形象，看起来就是「要刷新才更新」。 */
    function patchPetStatusCacheVariant(variant) {
        try {
            const cached = readPetStatusCache();
            if (!cached || !cached.pet) return;
            cached.pet.variant = normPetVariant(variant);
            writePetStatusCache(cached);
        } catch (e) { /* ignore */ }
    }
    ...
    function applyPetStatus(data, fromCache) {
        ...
        if (data.hasPet) {
            ...
            if (petState.pet) {
                ...
                petState.pet.variant = normPetVariant(petState.pet.variant);
                // 解锁清单以服务端为准：后端给了清单就照它显示，未解锁的形象才出现兑换按钮。
                // 只有后端没返回该字段（老版 worker 未部署）时才退回本地兜底，
                // 避免把整排形象误判成未解锁；本地缓存不再与后端做并集，
                // 否则一旦缓存里混进多余的形象键，界面会永远显示“已解锁”，兑换按钮再不出现。
                var serverUnlocked = petState.pet.unlocked_variants;
                if (Array.isArray(serverUnlocked) && serverUnlocked.length > 0) {
                    petState.pet.unlocked_variants = serverUnlocked.map(normPetVariant);
                } else {
                    var cachedUnlocked = loadUnlockedCache();
                    petState.pet.unlocked_variants = cachedUnlocked.length
                        ? cachedUnlocked.slice()
                        : [petState.pet.variant || 'default'];
                }
                // 当前形象与默认形象（雪妮）一定算已解锁，前者是正在用的，后者是初始形象
                [petState.pet.variant, 'default'].forEach(function (v) {
                    if (v && petState.pet.unlocked_variants.indexOf(v) < 0) petState.pet.unlocked_variants.push(v);
                });
                // 只保留注册表里真实存在的形象键，避免历史遗留键占用名额
                petState.pet.unlocked_variants = petState.pet.unlocked_variants.filter(function (v) {
                    return PET_VARIANTS.indexOf(v) >= 0;
                });
            ...
            if (data.pet && data.pet.variant) {
                localStorage.setItem('petCatVariant', normPetVariant(data.pet.variant));
            }
            updateEntryCard();
    ...
    // 形象代次：用户每成功切换一次形象 +1。请求发出时记下当时的值，
    // 回来时若已自增，说明这份响应里的形象是「切换之前」的旧值，不能拿它覆盖本地。
    let petVariantEpoch = 0;
    ...
    async function loadPetStatus() {
        ...
        const variantEpoch = petVariantEpoch;
        petStatusRequest = (async function () {
            try {
                const data = await petApi('/pet/status');
                if (epoch !== petStatusEpoch) return null;   // 已经退出/换账号，丢弃
                // 请求在飞的时候用户又切了形象：这份响应里的 variant 已经过时，
                // 直接铺下去会把桌面宠物 / 入口卡回滚成旧形象（刷新才恢复），
                // 所以以本地刚切好的形象为准，其余字段照旧用服务端的。
                if (variantEpoch !== petVariantEpoch && data && data.pet) {
                    const localVariant = normPetVariant(localStorage.getItem('petCatVariant'));
                    if (localVariant) data.pet.variant = localVariant;
                }
    ...
    function updateEntryCard() {
        ...
        if (!petState.hasPet && !petState.reason) {
            resetEntryCard();
            return;
        }

        if (petState.reason === 'not_adopted') {
            petEntryCard.classList.remove('hidden');
            petEntryCard.classList.add('clickable');
            if (petName) petName.textContent = '点击领养宠物';
            if (petLevel) petLevel.style.display = 'none';
            const statsEl = document.getElementById('petStats');
            if (statsEl) statsEl.style.display = 'none';
            const welcomeEl = document.getElementById('petEntryWelcome');
            if (welcomeEl) welcomeEl.textContent = '领养你的专属宠物';
            return;
        }
    ...
    function openPetPage() {
        ...
        // 进宠物页前先对一次形象：别的地方（另一个窗口 / 另一份缓存）刚切过形象时，
        // 这里能立刻跟上，而不是画出上一次的形象
        syncPetVariantOnShow();
    ...
    function closePetPage() {
        ...
        setTimeout(() => {
            ...
            pagePet.style.transform = '';
            // 回到 app 页面：先按当前形象重画一遍入口卡 / 悬浮宠物再让宠物出场，
            // 保证「在宠物页切过形象」这件事立刻反映到 app 页上（不必刷新）
            syncPetVariantOnShow();
            showFloatingPet();
        }, 350);
    }
    ...
    function getCatVariant() {
        if (petState.pet && petState.pet.variant && PET_VARIANTS.indexOf(normPetVariant(petState.pet.variant)) >= 0) {
            return normPetVariant(petState.pet.variant);
        }
        return normPetVariant(localStorage.getItem('petCatVariant')) || 'default';
    }
    ...
    function getUnlockedVariants() {
        if (petState.pet && Array.isArray(petState.pet.unlocked_variants) && petState.pet.unlocked_variants.length > 0) {
            return petState.pet.unlocked_variants.map(normPetVariant).filter(v => PET_VARIANTS.indexOf(v) >= 0);
        }
        // 状态还没铺下来时的兜底：默认形象（雪妮）不需要解锁，当前形象一定已解锁
        var fallback = ['default'];
        var cur = getCatVariant();
        if (cur && fallback.indexOf(cur) < 0) fallback.push(cur);
        return fallback;
    }

    // 已解锁形象清单的本地缓存：只作为「后端没返回清单（老版 worker）」时的兜底。
    // key 带版本号 —— 旧版缓存无法区分「服务器确认过的」和「界面曾误判的」，
    // 直接换名让历史脏数据失效，避免缓存把未解锁的形象一直显示成已解锁。
    const UNLOCKED_CACHE_KEY = 'petUnlockedVariantsV2';
    const UNLOCKED_CACHE_KEY_LEGACY = 'petUnlockedVariants';
    // 模块初始化时就把旧 key 删掉：它曾经把未解锁的形象一直显示成已解锁，
    // 留着也只会在「后端没返回清单」时被读到，直接清空更省心
    try { localStorage.removeItem(UNLOCKED_CACHE_KEY_LEGACY); } catch (e) { /* ignore */ }
    function cacheUnlockedVariants() {
        try {
            if (petState.pet && Array.isArray(petState.pet.unlocked_variants)) {
                const list = petState.pet.unlocked_variants.map(normPetVariant).filter(v => PET_VARIANTS.indexOf(v) >= 0);
                localStorage.setItem(UNLOCKED_CACHE_KEY, JSON.stringify(list));
                localStorage.removeItem(UNLOCKED_CACHE_KEY_LEGACY);
            }
        } catch (e) { /* ignore */ }
    }
    function loadUnlockedCache() {
        try {
            localStorage.removeItem(UNLOCKED_CACHE_KEY_LEGACY);   // 旧版缓存一律丢弃
            const raw = localStorage.getItem(UNLOCKED_CACHE_KEY);
            if (!raw) return [];
            const arr = JSON.parse(raw);
            if (!Array.isArray(arr)) return [];
            return arr.map(normPetVariant).filter(v => PET_VARIANTS.indexOf(v) >= 0);
        } catch (e) { return []; }
    }

    /* 合并解锁清单：服务器返回 ∪ 本地已确认 ∪ 本次明确解锁的键。
       不能直接用服务器返回值覆盖 —— 后端若因白名单/落库问题少返回了某个键，
       覆盖会把界面上本已解锁的形象“退回锁定”。 */
    function mergeUnlockedVariants(serverList, extraKeys) {
        const set = {};
        (Array.isArray(serverList) ? serverList : []).forEach(function (v) { set[normPetVariant(v)] = 1; });
        getUnlockedVariants().forEach(function (v) { set[v] = 1; });
        (Array.isArray(extraKeys) ? extraKeys : []).forEach(function (v) { set[normPetVariant(v)] = 1; });
        return Object.keys(set).filter(function (v) { return PET_VARIANTS.indexOf(v) >= 0; });
    }
    ...
    // 甩尾颜色/花纹取自 pets/*.svg（每只宠物尾巴的真实配色，见 CAT_TAIL_COLORS）。
    // 形状/姿态仍由 Python 逐帧从 swipe.gif 量出，注入到各宠物渲染处、压在猫咪本体之下。
    const CAT_TAIL_COLORS = {
        default:  { base: '#FFFFFF', tip: '#FFFFFF' }, // 默认/雪妮：白尾
        heitang:  { base: '#79645F', tip: '#79645F' }, // 黑糖：棕
        huihui:   { base: '#948F89', tip: '#948F89' }, // 灰灰：灰
        xueqiu:   { base: '#AF9982', tip: '#FFFFFF' }, // 雪球：棕褐 + 白尖
        xiaobai:  { base: '#FCFCED', tip: '#FCFCED' }, // 小白：白
        taozi:    { base: '#F8B075', tip: '#F8B075' }, // 桃子：桃色
        yuanyuan: { base: '#FEFFF9', tip: '#FEFFF9' }, // 圆圆：奶白
        buding:   { base: '#FEBA39', tip: '#FEBA39' }, // 布丁：橙
        huahua:   { base: '#FED899', tip: '#E8A056' }  // 花花：浅橙 + 橙纹
    };
    let __catSwipeUid = 0;
    function catSwipeMarkup(variant) {
        const uid = 'cs' + (++__catSwipeUid);
        const c = CAT_TAIL_COLORS[variant] || CAT_TAIL_COLORS.default;
        return `
            <svg class="cat-swipe-svg" viewBox="0 0 500 500" fill="none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
                <defs>
                    <linearGradient id="catSwipeGrad_${uid}" x1="310" y1="366" x2="432" y2="241" gradientUnits="userSpaceOnUse">
                        <stop offset="0%" stop-color="${c.base}"/>
                        <stop offset="100%" stop-color="${c.tip}"/>
                    </linearGradient>
                </defs>
                <path class="cat-swipe-whip" fill="url(#catSwipeGrad_${uid})" d="M310.0 363.8C310.6 361.7 311.9 355.1 313.6 351.0C315.4 346.9 318.3 343.0 320.8 339.4C323.2 335.7 325.7 332.5 328.2 329.2C330.8 325.8 333.4 322.4 336.0 319.2C338.7 316.0 341.4 313.1 344.1 310.1C346.8 307.1 349.4 304.2 352.1 301.2C354.8 298.1 357.6 294.9 360.3 291.8C363.1 288.7 365.8 285.6 368.5 282.7C371.2 279.7 373.7 276.9 376.4 274.1C379.1 271.2 381.8 268.4 384.6 265.6C387.4 262.8 390.2 260.1 393.1 257.3C396.1 254.6 399.1 252.0 402.1 249.3C405.2 246.7 409.9 242.8 411.5 241.5C417.3 235.7 426.6 235.7 432.4 241.5C438.2 247.3 438.2 256.6 432.4 262.4C431.1 263.9 427.3 268.5 424.8 271.5C422.2 274.6 419.8 277.8 417.1 280.8C414.4 283.8 411.5 286.7 408.6 289.6C405.7 292.5 402.9 295.3 399.8 298.1C396.8 300.8 393.5 303.5 390.3 306.0C387.1 308.6 383.8 310.7 380.7 313.1C377.6 315.6 374.8 318.1 371.9 320.7C368.9 323.4 366.1 326.3 363.1 329.0C360.1 331.7 357.0 334.2 354.1 336.8C351.2 339.3 348.3 341.8 345.5 344.4C342.8 347.1 340.1 350.0 337.6 352.7C335.1 355.3 332.4 357.4 330.3 360.3C328.3 363.1 326.1 368.2 325.2 369.8C323.6 374.0 310.5 372.8 314.7 374.4C310.5 372.8 308.4 368.0 310.0 363.8Z"></path>
            </svg>`;
    }
    ...
    function bindPetBallEvents() {
        ...
        ball.addEventListener('click', (e) => {
            ...
            const container = ball.closest('.pet-cat-container');
            if (container) {
                if (_swipeTimer) { clearTimeout(_swipeTimer); _swipeTimer = null; }
                container.classList.remove('swiping');
                var whip = container.querySelector('.cat-swipe-whip');
                if (whip) {
                    whip.style.animation = 'none';
                    void whip.offsetWidth;
                    whip.style.animation = '';
                }
                void container.offsetWidth;
                container.classList.add('swiping');
                _swipeTimer = setTimeout(() => { container.classList.remove('swiping'); _swipeTimer = null; }, 2000);
            }
    ...
    function showPetConfirm(title, desc, confirmText) {
            ...
            overlay.innerHTML = `
                <div class="pet-confirm-modal">
                    ...
                    <div class="pet-confirm-actions">
                        ...
                        <button class="pet-confirm-btn confirm">${confirmText || '确认兑换'}</button>
                    </div>
                </div>
            `;
            document.body.appendChild(overlay);
    ...
    function showShopModal() {
        ...
        // 打开商城时静默对一次服务端状态：内存里的解锁清单可能来自本地缓存或旧会话，
        // 拉取完成后重算卡片状态，保证「未解锁的形象一定带兑换按钮」。
        if (petState.hasPet) {
            loadPetStatus().then(function () {
                if (shopModalOpen && shopCurrentTab === 'skin' && document.getElementById('catSkinGrid')) {
                ...
                <div class="pet-cat-skin-grid" id="catSkinGrid">
                    ${PET_VARIANTS.map(v => `
                        <button class="pet-cat-skin-card ${getCatVariant() === v ? 'active' : ''}" data-variant="${v}">
                            ...
                            <span class="pet-cat-skin-name">${variantNameHtml(v)}</span>
                        </button>
                    `).join('')}
                </div>
            `;
    ...
    function bindShopSkinEvents() {
        ...
        grid.addEventListener('click', async function (e) {
            ...
            try {
                ...
                if (petState.pet) {
                    // 解锁清单以服务器返回为准，只做并集防缩水、不凭空认账：
                    // 后端若没登记该形象，界面就该如实显示未解锁，避免出现“看着已解锁其实服务器没有”的假状态
                    petState.pet.unlocked_variants = mergeUnlockedVariants(res.unlocked_variants);
                    petState.pet.points = res.points != null ? res.points : (Math.max(0, curPoints - PET_SKIN_COST));
                }
                if (getUnlockedVariants().indexOf(variant) < 0) {
                    showGameTip('兑换未生效：后端未登记「' + (PET_VARIANT_NAMES[variant] || variant) + '」', 'warn');
                } else {
        ...
        if (bagCurrentTab === 'skin') {
                    ...
                    ${PET_VARIANTS.map(v => {
                        ...
                        return `
                            <div class="pet-bag-skin-card ${isActive ? 'active' : ''} ${isUnlocked ? '' : 'locked'}" data-variant="${v}">
                                ...
                                <span class="pet-bag-skin-name">${variantNameHtml(v)}</span>
            ...
            if (window.CatSVG) {
                PET_VARIANTS.forEach(v => {
                    ...
                    if (el) el.innerHTML = window.CatSVG('pet-cat-skin-svg', v) + catSwipeMarkup(v);
                });
            }
            // 绑定切换事件
    ...
    function refreshSkinCardStates() {
        ...
        grid.querySelectorAll('.pet-cat-skin-card').forEach(function (card) {
            ...
            if (isLocked) {
                ...
                exchangeBtn.classList.remove('paid-btn');
                if (enough) {
    ...
    function renderSkinPreviews() {
        ...
        PET_VARIANTS.forEach(function (variant) {
            ...
            if (el) el.innerHTML = window.CatSVG('pet-cat-skin-svg', variant) + catSwipeMarkup(variant);
        });
    }
    ...
    /* 后端明确拒绝（未解锁等 4xx）或改写了形象键时，把界面回滚到切换前的形象。
       否则会出现「点一下猫变了、刷新又变回去」的假成功。 */
    function rollbackCatVariant(prevVariant) {
        if (!prevVariant) return;
        if (petState.pet) petState.pet.variant = prevVariant;
        localStorage.setItem('petCatVariant', prevVariant);
        updateActiveSkinCard(prevVariant);
        switchCatVariant(prevVariant);
        refreshSkinCardStates();
    }

    async function switchCatVariantWithBackend(variant) {
        const prevVariant = getCatVariant();
        ...
        // 本地宠物状态缓存一并改掉：否则之后任何一次「缓存优先」渲染都会画回旧形象
        patchPetStatusCacheVariant(variant);
        updateActiveSkinCard(variant);
        switchCatVariant(variant);
        try {
            const res = await petApi('/pet/skin/switch', 'POST', { variant });
            // 后端把形象键改写成了别的（典型：旧版后端不认识新键，会归一化成 default）
            if (res && res.variant && res.variant !== variant) {
                console.warn('后端改写了形象键：', variant, '->', res.variant);
                rollbackCatVariant(prevVariant);
                showGameTip('后端未识别「' + (PET_VARIANT_NAMES[variant] || variant) + '」，切换未生效', 'warn');
                return res;
            }
            if (petState.pet && res && res.unlocked_variants) {
                petState.pet.unlocked_variants = mergeUnlockedVariants(res.unlocked_variants, [variant]);
                cacheUnlockedVariants();
            }
            // 服务端确认后再补一次缓存，保证「内存 = 界面 = 缓存 = 服务端」一致
            patchPetStatusCacheVariant(variant);
            return res;
        } catch (e) {
            console.warn('切换形象后端同步失败:', e);
            // 业务性失败（未解锁、无宠物等）回滚，网络类故障保留本地切换
            if (e && e.status >= 400 && e.status < 500) {
                rollbackCatVariant(prevVariant);
                showGameTip(e.message || '切换失败', 'warn');
            }
            return null;
        }
    }
    ...
    /* 把「当前该显示的猫形象」一次性铺到所有展示位：
       ① 游戏页大猫 ② 入口卡头像 ③ 桌面悬浮宠物。
       幂等：每次都以 getCatVariant() 为准重画，并且只在「画的东西和要的不一致」时才动 DOM，
       所以任何一条路径（商城兑换 / 背包切换 / 状态刷新 / 从宠物页返回）漏发切换事件,
       下一次调用都会自动纠正，不会出现「要刷新才更新」的旧形象残留。
       variant 传空时取当前形象（正常调用都不传）。 */
    function applyPetVariantEverywhere(variant) {
        if (!window.CatSVG || typeof window.CatSVG !== 'function') return;
        variant = normPetVariant(variant || getCatVariant()) || 'default';
        var accent = getCatAccent(variant);

        
        var ball = document.querySelector('.pet-slime');
        if (ball && ball.getAttribute('data-variant') !== variant) {
        ...
        // 入口卡交给 injectEntryCardCat（它按 data-cat-variant 判断是否需要重画，
        // 未领养时保持 adopt.svg 占位图）
        injectEntryCardCat(variant);

        // 桌面悬浮宠物同步切换形象
        updateFloatingPetVariant(variant);
    }

    function switchCatVariant(variant) {
        // 本地已经切好了：记一代，供晚到的 /pet/status 判断「这份数据是不是旧的」
        petVariantEpoch++;
        applyPetVariantEverywhere(variant);
    }

    /* 回到 app 页面 / 页面重新可见时的兜底对账：
       形象可能在别处被改过（宠物页切完返回、同一个 localStorage 的另一个窗口或
       另一个 WebView 里切换过），这里以 localStorage 里那份「最后一次切换结果」为准，
       和内存状态对齐后重画一遍所有展示位。 */
    function syncPetVariantOnShow() {
        try {
            var stored = normPetVariant(localStorage.getItem('petCatVariant'));
            if (stored && PET_VARIANTS.indexOf(stored) >= 0) {
                if (petState.pet && petState.pet.variant !== stored) petState.pet.variant = stored;
                applyPetVariantEverywhere(stored);
            } else {
                applyPetVariantEverywhere();
            }
        } catch (e) { /* ignore */ }
    }

    /* 跨窗口 / 跨文档切换形象时，localStorage 的 storage 事件会在**其他**文档里触发，
       这正是「宠物页切完，另一边 app 页的悬浮宠物和入口卡没跟上」的补救通道。 */
    function bindPetVariantSyncWatchers() {
        window.addEventListener('storage', function (e) {
            if (e && e.key === 'petCatVariant') syncPetVariantOnShow();
        });
        // bfcache 恢复 / 从后台切回前台：兜底重画一次
        window.addEventListener('pageshow', function () { syncPetVariantOnShow(); });
        document.addEventListener('visibilitychange', function () {
            if (!document.hidden) syncPetVariantOnShow();
        });
    }
    ...
    function showAdoptModal() {
        ...
        modal.innerHTML = `
            <div class="pet-setup-modal">
                ...
                <div class="pet-setup-preview">
                    ...
                    <div class="pet-setup-preview-name">${variantNameHtml('default')}</div>
                </div>
        ...
        if (previewEl && window.CatSVG) {
            previewEl.innerHTML = window.CatSVG('pet-adopt-preview-svg', 'default') + catSwipeMarkup('default');
        }
    ...
    function createFloatingPet() {
        ...
        const variant = normPetVariant(getCatVariant()) || 'default';
        floatingPetEl.innerHTML = `
            <div class="pet-floating-inner${floatingFacingFlipped ? ' flipped' : ''}" data-variant="${variant}" style="--cat-accent:${getCatAccent(variant)}">
                ${catSwipeMarkup(variant)}
    ...
    function showFloatingPet() {
        ...
        if (floatingPetEl) {
            // 显示前强制按当前形象重绘：任何路径（商城兑换/皮肤面板/背包切换/状态刷新）
            // 漏发切换事件时，这里都会兜底，杜绝桌面宠物残留旧形象、刷新才更新的问题
    ...
    // variant 传空时取当前形象；内部按「已经画的是哪个形象」判断要不要重画，
    // 所以重复调用是安全的，也不会白白重启头像的眨眼动画
    function updateFloatingPetVariant(variant) {
        if (!floatingPetEl) return;
        if (!window.CatSVG) return;
        variant = normPetVariant(variant || getCatVariant()) || 'default';
        const inner = floatingPetEl.querySelector('.pet-floating-inner');
        if (!inner) return;
        // 形象没变且 SVG 还在：不动 DOM（避免重复替换把动画/朝向重置）
        if (inner.getAttribute('data-variant') === variant && inner.querySelector('.pet-cat-avatar-svg')) return;
    ...
    // ===== 初始化 =====
    // variant 传空时取当前形象。用 data-cat-variant 记录「卡里已经画的是哪个形象」，
    // 一旦和当前形象不一致就重画 —— 只认「注入过没有」的话，切完形象这张卡会一直是旧猫，
    // 必须刷新页面才更新。
    function injectEntryCardCat(variant) {
        ...
        if (!petState.hasPet || !petState.pet) {
            ...
            slime.removeAttribute('data-catVariant');
        ...
        if (!(window.CatSVG && typeof window.CatSVG === 'function')) return;
        variant = normPetVariant(variant || getCatVariant()) || 'default';
        // 已经画的就是这个形象（且 SVG 还在）：不用动
        if (slime.dataset.catVariant === variant && slime.querySelector('.pet-cat-avatar-svg')) return;
        ...
        slime.dataset.catVariant = variant;
    }
    ...
    function initPet() {
        ...
        // 跨窗口 / 跨文档 / 回到前台的兜底同步（只绑一次）
        bindPetVariantSyncWatchers();
    }
    ...
    window.PetSystem = {
        ...
        // 形象一次性铺到所有展示位（游戏页大猫 / 入口卡 / 悬浮宠物），幂等，可随时调用
        applyVariant: applyPetVariantEverywhere,
        // 回到 app 页面 / 页面可见时的兜底对账
        syncVariant: syncPetVariantOnShow,
        getState: () => petState,
