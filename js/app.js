...
async function registerWithCode(email, password, nickname, code) {
    const data = await apiCall('/auth/register', 'POST', { email, password, nickname, code });
    return data;
}
...
async function unbindPartnerApi() {
    ...
    function logout() {
        ...
        if (window.OAuthLogin && typeof OAuthLogin.clearAuth === 'function') {
            OAuthLogin.clearAuth();
        }
...
$('#loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        showOAuthError(err.message || '登录失败');
...
// ===== OAuth 第三方登录（QQ / 微信）=====
// ── 登录加载动画控制（邮箱/QQ/微信通用）──
var oauthLoadingOverlay = document.getElementById('oauthLoadingOverlay');
var oauthLoadingSpinner = document.getElementById('oauthLoadingSpinner');
var oauthLoadingText = document.getElementById('oauthLoadingText');

function showOAuthLoading(text) {
    if (!oauthLoadingOverlay) return;
    oauthLoadingSpinner.className = 'oauth-loading-spinner';
    oauthLoadingText.textContent = text || '正在登录...';
    oauthLoadingOverlay.style.display = 'flex';
}
function showOAuthSuccess() {
    if (!oauthLoadingOverlay) return;
    oauthLoadingSpinner.className = 'oauth-loading-spinner success';
    oauthLoadingText.textContent = '登录成功';
    setTimeout(function() { oauthLoadingOverlay.style.display = 'none'; }, 600);
}
function showOAuthError(msg) {
    if (!oauthLoadingOverlay) return;
    oauthLoadingSpinner.className = 'oauth-loading-spinner error';
    oauthLoadingText.textContent = msg || '登录失败';
    setTimeout(function() {
        oauthLoadingOverlay.style.display = 'none';
        oauthLoadingSpinner.className = 'oauth-loading-spinner';
    }, 1500);
}
...
function updateOAuthLoadingText(text) {
    if (!oauthLoadingText) return;
    oauthLoadingText.textContent = text || '';
}

if (window.OAuthLogin) {
    OAuthLogin.init({
        onSuccess: async function(oauthToken, oauthUser) {
            showOAuthSuccess();
            token = oauthToken;
        ...
        onError: function(msg) {
            showOAuthError(msg);
            showToast(msg || '第三方登录失败');
        },
    });

    // 绑定 QQ / 微信 按钮
    const socialWechat = document.querySelector('.social-wechat');
    const socialQQ = document.querySelector('.social-qq');
    if (socialWechat) {
        socialWechat.addEventListener('click', function() {
            const agreeCheckbox = document.getElementById('agreeCheckbox');
            if (agreeCheckbox && !agreeCheckbox.checked) {
                showToast('请先阅读并同意协议');
                return;
            }
            showOAuthLoading('正在拉起微信...');
            OAuthLogin.start('wechat');
        });
    }
    if (socialQQ) {
        socialQQ.addEventListener('click', function() {
            const agreeCheckbox = document.getElementById('agreeCheckbox');
            if (agreeCheckbox && !agreeCheckbox.checked) {
                showToast('请先阅读并同意协议');
                return;
            }
            showOAuthLoading('正在拉起QQ...');
            OAuthLogin.start('qq');
        });
    }

    // 同标签跳转兜底：移动端浏览器可能拦截弹窗，OAuth 在同标签完成后
    // 回调页面把 code 存在 sessionStorage，这里检测并自动登录
    (function checkOAuthRedirect() {
        try {
            var pendingCode = sessionStorage.getItem('oauth_pending_code');
            var pendingState = sessionStorage.getItem('oauth_pending_state');
            var pendingProvider = sessionStorage.getItem('oauth_pending_provider');
            if (pendingCode && pendingProvider) {
                ...
                OAuthLogin.handleCode(pendingProvider, pendingCode, pendingState);
            }
        } catch (e) {}
    })();
}
...
$('#registerForm').addEventListener('submit', async (e) => {
    ...
    try {
        await registerWithCode(email, password, nickname, code);
        showToast('注册成功，请登录');
        showLoginPage();
...
function updateAccountSecurityStatus() {
    ...
    var changePwdItem = document.getElementById('accountChangePasswordItem');
    var isOAuthUser = currentUser && currentUser.email && currentUser.email.endsWith('@oauth.local');

    if (changePwdItem) {
        changePwdItem.style.display = isOAuthUser ? 'none' : '';
    }
...
function initChangePasswordEvents() {
    ...
    document.getElementById('regSendCodeBtn')?.addEventListener('click', async function() {
        const email = $('#regEmail').value.trim();
        if (!email) {
            showToast('请先输入邮箱');
            return;
...
// ===== 赞助作者弹窗 =====
(function () {
    const sponsorBtn = document.getElementById('aboutSponsorBtn');
    const sponsorOverlay = document.getElementById('sponsorOverlay');
    const sponsorClose = document.getElementById('sponsorClose');
    if (!sponsorBtn || !sponsorOverlay) return;

    function openSponsor() {
        sponsorOverlay.classList.add('active');
        requestAnimationFrame(() => sponsorOverlay.classList.add('show'));
    }
    function closeSponsor() {
        sponsorOverlay.classList.remove('show');
        setTimeout(() => sponsorOverlay.classList.remove('active'), 260);
    }
    ...
    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && sponsorOverlay.classList.contains('show')) closeSponsor();
    });
})();
